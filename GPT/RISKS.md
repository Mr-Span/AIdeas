...
| R-010 | A partially completed multi-role round is resumed with inconsistent context after restart | contradictory or stale proposals | explicit retry copies only completed role artifacts into a new round bound to the original digest; incomplete roles rerun and all concurrent cleanup settles before reconciliation |
...
| R-012 | A worker edits secrets, local data or files outside its task packet | irreversible disclosure or unrelated code damage | external worktree, server-owned file scopes, forbidden path list, diff secret scan and protected-checkout readback before commit |
| R-013 | A stale/expired worker finalizes after retry | duplicate or incorrect accepted result | immutable ContextPacket, lease token, expiry and fenced terminal update; restart marks active attempts expired and preserves the worktree |
| R-014 | Git automation pushes or merges a different commit than the reviewed one | evidence bypass or regression | action policy uses EvidenceBundle commit/diff/base digests; branch/remote/PR/merge readback and durable integration receipt |
